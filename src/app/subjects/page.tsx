  ...
  return (
    <div className="p-8 lg:p-12">
      ...
      <div className="mb-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          {activeTab === "decks" && !(loaded && allBundles.length === 0) && (
            <div className="flex gap-2">
              ...
              <Button variant="secondary" onClick={() => document.getElementById("lib-share-import")?.click()}>{t("subjnew.importShare")}</Button>
        ...
        {/* Tabs */}
        <div role="tablist" aria-label={t("subj.sections")} onKeyDown={(e) => {
      ...
      {!loaded ? (
        ...
        <div className="space-y-8">
          <EmptyState
            ...
            title={t("subjnew.noSubjectsYet")}
            description={t("subjnew.subjectsOrganize")}
            action={
              <Button onClick={() => setModalOpen(true)}>
                <Plus size={16} />
                {t("subjnew.createSubject")}
              </Button>
            }
          />
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      aria-label={t("subj.deleteSubject")}
            ...
            {allBundles.length > 3 && (
              <div className="mb-4 flex items-center gap-2">
                <div className="relative flex-1 max-w-sm">
                  <Search size={14} className="absolute start-3 top-1/2 -translate-y-1/2 text-muted-fg" aria-hidden />
                  <input
                    value={deckSearch}
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <div className="flex gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100 max-md:opacity-100">
                          ...
                          <button
                            ...
                            aria-label={t("subj.deleteDeck")}
                            title={t("subj.deleteDeck")}
            ...
            <div className="space-y-6">
              <div className="glass rounded-2xl p-6">
                ...
                {allBundles.length > 0 ? (
                  ...
                  >
                    <button onClick={() => router.push(`/bundles/${b.id}/cards`)} className="flex-1 flex items-center justify-between text-start min-w-0">
                  ...
                  <p className="mt-6 text-sm text-muted-fg">{t("subjnew.noDecksYet")}</p>
                )}
              </div>
              <div className="glass rounded-2xl p-6">
 ...
 (replaces native confirm()) */}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <Input
              placeholder={t("subjnew.newTopicName")}
              value={newTopicName}
          ...
          {managedTopics.length > 3 && (
            <div className="relative">
              ...
              <input
                placeholder={t("subjnew.searchTopics")}
                value={topicSearch}
                onChange={(e) => setTopicSearch(e.target.value)}
                className="h-10 w-full rounded-xl border border-border bg-bg ps-8 pe-3 text-sm font-medium tracking-tight placeholder:text-muted-fg/60 focus:outline-none"
              />
            </div>
          )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={t("modal.newSubject")}>
        <div className="space-y-6">
          <Input
            ...
            placeholder={t("subjnew.exampleSubject")}
            value={name}
            onChange={(e) => setName(e.target.value)}
          />
          <Input
            label={t("subjnew.descriptionOptional")}
            placeholder={t("subjnew.briefDescription")}
            value={description}
      ...
      {/* Edit Subject Modal */}
      <Modal open={!!editSubject} onClose={() => setEditSubject(null)} title={t("subj.editSubject")}>
        {editSubject && (
          <div className="space-y-6">
            <Input
              label={t("subjnew.subjectName")}
      ...
      <Modal open={!!deckEditBundle} onClose={() => setDeckEditBundle(null)} title={t("subj.editDeck")}>
        {deckEditBundle && (
          <div className="space-y-6">
            <Input label={t("subjnew.deckName")} value={deckEditName} onChange={(e) => setDeckEditName(e.target.value)} />
      ...
      <Modal open={!!deckDeleteTarget} onClose={() => setDeckDeleteTarget(null)} title={t("subj.deleteDeck")}>
        {deckDeleteTarget && (
          <div className="space-y-6">
            <p className="text-sm text-muted-fg">
